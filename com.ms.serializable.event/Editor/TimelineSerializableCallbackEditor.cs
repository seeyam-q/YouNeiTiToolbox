...
namespace MS.Events.Editor{
    ...
    [CustomPropertyDrawer(typeof(TimelineSerializableCallback))]
    internal class TimelineSerializableCallbackEditor : PropertyDrawer
    {
        ...
        private void DrawMethod(Rect rect, SerializedProperty property){
            var callback = EditorHelper.GetTargetObjectOfProperty(property) as TimelineSerializableCallback;
            ...
            if(!callback.target || string.IsNullOrEmpty(funcName)){
                displayFuncName = "No Function";
            }else if(!callback.Validate()){
                displayFuncName = $"Missing<{callback.target.GetType().Name}.{funcName}>";
            }else{
                ...
                if(validateResult == MethodInvalidReason.None){
                    displayFuncName = GetFormatedMethodName(callback.target,funcName);
                }else{
                    displayFuncName = $"<{callback.target.GetType().Name}.{funcName}> invalid with reason: {validateResult}";
                }
            }
            EditorGUI.BeginDisabledGroup(callback.target == null);
            if(GUI.Button(rect, displayFuncName, EditorStyles.popup)){
                ShowCallbackSelector(callback.target,property);
            }
            EditorGUI.EndDisabledGroup();
        }

        public override void OnGUI(Rect rect, SerializedProperty property, GUIContent label){
            var timelineCallback = EditorHelper.GetTargetObjectOfProperty(property) as TimelineSerializableCallback;
            var targetObject = timelineCallback.target;
            var funcName = property.FindPropertyRelative("_funcName").stringValue;
            ...
            var identicalComponentIndex = CalculateIdenticalComponentIndex(targetObject);
