...
namespace FortySevenE
{
	...
	{
		private SerializedProperty actionProp, conditionProp, customComponentNameProp, customBooleanFieldNameProp;

		private void OnEnable()
		{
			actionProp = serializedObject.FindProperty("action");
			conditionProp = serializedObject.FindProperty("condition");
			customComponentNameProp = serializedObject.FindProperty("customComponentName");
			customBooleanFieldNameProp = serializedObject.FindProperty("customBooleanFieldName");
		...
		public override void OnInspectorGUI()
		{
			...
			if (!isMarker)
			{
				...
				switch (conditionType)
				{
					...
					case TimeMachineBehaviour.Condition.CustomBooleanField:
						EditorGUILayout.HelpBox(
							"The above action will be executed if any Unit in the Platoon connected below is alive when the playhead reaches this clip.",
							MessageType.Info);
						EditorGUILayout.Space();
						EditorGUILayout.PropertyField(conditionProp);
						EditorGUILayout.PropertyField(serializedObject.FindProperty("target"));
						EditorGUILayout.PropertyField(customComponentNameProp);
						EditorGUILayout.PropertyField(customBooleanFieldNameProp);
						break;
				}
			}
